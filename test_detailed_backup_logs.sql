...
VALUES 
    ('Test détaillé 1', 100.50, 'Note de test pour logs détaillés', 'haute', 'depense', 'en_attente'),
    ('Test détaillé 2', 250.00, 'Autre note de test', 'moyenne', 'depense', 'en_attente'),
    ('Test détaillé 3', 75.25, 'Troisième note', 'basse', 'depense', 'en_attente')
...
BEGIN
    ...
    backup_name := 'test_detailed_backup_' || to_char(now(), 'YYYY-MM-DD_HH24-MI-SS');
    ...
    VALUES ('started', 'Début de la sauvegarde détaillée: ' || backup_name, now());
    ...
    VALUES (
        'success', 
        'Sauvegarde détaillée confirmée: ' || backup_name || ' | Tables: ' || 
    ...
    RAISE NOTICE 'Total: % enregistrements, % KB', total_records, ROUND(total_size/1024.0, 1);
END $$;
...
SELECT 
    ...
    backup_id
FROM backup_logs 
WHERE message LIKE '%détaillé%' OR message LIKE '%Table%'
