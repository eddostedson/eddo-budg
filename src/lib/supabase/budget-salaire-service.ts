...
export const BudgetSalaireService = {
  ...
  async updateRubrique(rubriqueId: string, updates: {
    nom?: string
    montantBudgete?: number
    typeDepense?: TypeDepenseSalaire
    ...
    if (updates.montantBudgete !== undefined) payload.montant_budgete = updates.montantBudgete
    if (updates.typeDepense !== undefined) payload.type_depense = updates.typeDepense
    ...
    if (error) {
      console.error('❌ Erreur mise à jour rubrique budget salaire:', error)
      return false
    }
  ...
  async deleteRubrique(rubriqueId: string): Promise<boolean> {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return false
    ...
    if (error) {
      console.error('❌ Erreur suppression rubrique budget salaire:', error)
      return false
    }
