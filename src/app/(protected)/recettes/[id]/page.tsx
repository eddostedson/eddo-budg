...
const RecetteDetailsPage: React.FC = () => {
  ...
  const { depenses, loading: depensesLoading, createDepense, updateDepense, deleteDepense, refreshDepenses } = useDepenses()
  ...
  useEffect(() => {
    if (!recettesLoading && !depensesLoading) {
      ...
      if (foundRecette) {
        ...
        // Trouver les dépenses liées par recette_id
        const depensesLiees = depenses.filter(d => {
          const correspondanceParId = d.recetteId === recetteId
          console.log('🔍 Dépense:', d.libelle, 'Montant:', d.montant, 'recetteId:', d.recetteId, 'Recette courante:', recetteId, 'Correspondance:', correspondanceParId)
          return correspondanceParId
        })
        
        setDepensesLiees(depensesLiees)
        console.log('🔍 Dépenses liées trouvées:', depensesLiees.length)
        console.log('🔍 Dépenses liées:', depensesLiees)
      } else {
  ...
  const handleAddDepense = () => {
    if (soldeDisponibleCalcule === 0) {
      toast.error('Impossible d\'ajouter une dépense : le solde disponible est à 0')
      return
    }
    setCreateForm({
      libelle: '',
  ...
  const handleSaveCreate = async () => {
    ...
    try {
      ...
      const success = await createDepense({
        userId: '', // Sera défini par le contexte
        libelle: createForm.libelle,
        montant: montant,
        date: createForm.date,
        description: createForm.description || '',
        recetteId: recette.id, // Lier à la recette courante
  ...
  // Fonction supprimée - on calcule maintenant directement avec soldeDisponibleCalcule

  const getStatusColor = () => {
    if (pourcentageDisponible >= 80) return 'bg-green-100 text-green-700 border-green-300'
    if (pourcentageDisponible >= 50) return 'bg-blue-100 text-blue-700 border-blue-300'
    if (pourcentageDisponible >= 20) return 'bg-orange-100 text-orange-700 border-orange-300'
  ...
  const soldeDisponibleCalcule = recette.montant - totalDepenses
      ...
      <div className="max-w-7xl mx-auto p-8">
        ...
        >
          ...
          <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-xl">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{formatCurrency(soldeDisponibleCalcule)}</div>
              <div className="flex items-center mt-2">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <Button
              onClick={handleAddDepense}
              className={soldeDisponibleCalcule === 0 
                ? 'bg-gray-400 hover:bg-gray-400 cursor-not-allowed opacity-60' 
                : 'bg-blue-600 hover:bg-blue-700'
              }
              disabled={soldeDisponibleCalcule === 0}
              title={soldeDisponibleCalcule === 0 ? 'Impossible d\'ajouter une dépense : solde à 0' : ''}
            >
              <PlusIcon className="h-4 w-4 mr-2" />
              Ajouter une Dépense
            </Button>
            ...
            >
              ...
              <div className="space-y-6">
                ...
                {(() => {
                  ...
                  const montantSaisi = parseFloat(createForm.montant) || 0
                  const soldeActuel = recette.montant - totalDepenses
                  const soldeRestant = soldeActuel - montantSaisi
                  const estNegatif = soldeRestant < 0
                  
                  return (
                    <div className={`p-4 rounded-lg border ${estNegatif ? 'bg-red-50 border-red-300' : 'bg-blue-50 border-blue-300'}`}>
                      <div className="flex items-center justify-between text-sm mb-2">
                        <span className="font-medium text-gray-700">💰 Solde disponible :</span>
                        <span className="font-bold text-lg">{soldeActuel.toLocaleString()} F CFA</span>
                      </div>
                      {montantSaisi > 0 && (
                        <>
                          <div className="flex items-center justify-between text-sm mt-2">
                            <span className="text-gray-600">Montant saisi :</span>
                            <span className="text-gray-800 font-semibold">- {montantSaisi.toLocaleString()} F CFA</span>
                          </div>
                          <div className="border-t border-gray-300 my-3"></div>
                          <div className={`flex items-center justify-between font-bold ${estNegatif ? 'text-red-600' : 'text-green-600'}`}>
                            <span className="text-base">📊 Solde restant :</span>
                            <span className="text-xl">{soldeRestant.toLocaleString()} F CFA</span>
                          </div>
                          {estNegatif && (
                            <div className="mt-3 p-2 bg-red-100 border border-red-400 rounded text-xs text-red-700 flex items-start">
                              <span className="mr-2">⚠️</span>
                              <span>Le montant saisi dépasse le solde disponible de la recette !</span>
                            </div>
                          )}
                        </>
                      )}
                ...
                <div className="bg-blue-50 rounded-lg p-4">
                  ...
                  <div className="text-sm text-blue-700">
                    ...
                    <p><strong>Solde disponible:</strong> {formatCurrency(soldeDisponibleCalcule)}</p>
                  </div>
                </div>
              </div>
