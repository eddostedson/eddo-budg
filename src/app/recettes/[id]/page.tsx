...
export default function RecetteDetailPage() {
  ...
  const { recettes, updateRecette, refreshRecettes, toggleBankValidation } = useRecettes()
  ...
  const soldeDisponible = recette ? recette.montant - totalDepenses : 0
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-gray-800 mb-1">
                          Conformité du solde bancaire
                        </p>
                        <p className="text-xs text-gray-600">
                          {recette.validationBancaire 
                            ? 'Cette recette a été validée comme conforme à votre solde bancaire'
                            : 'Marquer cette recette comme conforme à votre solde bancaire'
                          }
                        </p>
                        {recette.validationBancaire && recette.dateValidationBancaire && (
                          <p className="text-xs text-yellow-700 mt-1">
                            Validé le {new Date(recette.dateValidationBancaire).toLocaleDateString('fr-FR')} à {new Date(recette.dateValidationBancaire).toLocaleTimeString('fr-FR')}
                          </p>
                        )}
                      </div>
                      <BankValidationBadge 
