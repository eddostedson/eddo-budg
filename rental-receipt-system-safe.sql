-- =====================================================
-- 🏠 SYSTÈME DE RECEIPTS DE LOYER AUTOMATIQUE (VERSION SÛRE)
-- =====================================================
...
-- Version qui gère les objets existants

-- =====================================================
-- 1. MODIFIER LA TABLE RECETTES POUR SUPPORTER LES LOYERS
-- =====================================================

-- Ajouter des colonnes pour la gestion locative (si elles n'existent pas)
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'is_rental') THEN
        ...
        RAISE NOTICE '✅ Colonne is_rental ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne is_rental existe déjà';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'property_id') THEN
        ...
        RAISE NOTICE '✅ Colonne property_id ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne property_id existe déjà';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'tenant_id') THEN
        ...
        RAISE NOTICE '✅ Colonne tenant_id ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne tenant_id existe déjà';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'contract_id') THEN
        ...
        RAISE NOTICE '✅ Colonne contract_id ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne contract_id existe déjà';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'rental_month') THEN
        ...
        RAISE NOTICE '✅ Colonne rental_month ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne rental_month existe déjà';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'rental_year') THEN
        ...
        RAISE NOTICE '✅ Colonne rental_year ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne rental_year existe déjà';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'rental_period_start') THEN
        ...
        RAISE NOTICE '✅ Colonne rental_period_start ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne rental_period_start existe déjà';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recettes' AND column_name = 'rental_period_end') THEN
        ...
        RAISE NOTICE '✅ Colonne rental_period_end ajoutée';
    ELSE
        RAISE NOTICE 'ℹ️ Colonne rental_period_end existe déjà';
    END IF;
END $$;

-- Créer les index pour les requêtes de loyers (si ils n'existent pas)
...
-- Créer les index pour les requêtes fréquentes (si ils n'existent pas)
...
DROP POLICY IF EXISTS "Users can view their own rental income links" ON rental_income_links;
DROP POLICY IF EXISTS "Users can insert their own rental income links" ON rental_income_links;
DROP POLICY IF EXISTS "Users can update their own rental income links" ON rental_income_links;
DROP POLICY IF EXISTS "Users can delete their own rental income links" ON rental_income_links;
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'rental_income_links') 
    THEN '✅ Table rental_income_links créée'
    ELSE '❌ Table rental_income_links manquante'
  END as table_status,
  CASE 
    WHEN EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'extract_rental_period_from_libelle') 
    THEN '✅ Fonction extract_rental_period_from_libelle créée'
    ELSE '❌ Fonction extract_rental_period_from_libelle manquante'
  END as function_status,
  CASE 
    WHEN EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_process_rental_income') 
    THEN '✅ Trigger trigger_process_rental_income créé'
    ELSE '❌ Trigger trigger_process_rental_income manquant'
